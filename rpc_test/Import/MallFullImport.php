...
class MallFullImport implements LoadSourceData{
    public function getIds($flag , $num = 50){
        $hash_ids = JMutil::retryClient('JumeiProduct_Search_Read_Deals', 'getPidOfJumeiMallByLimit', array($flag,$num));
        ImportWrite::log_info("type:mall,flag:{$flag},num:{$num},ids:{$hash_ids}");
        return $hash_ids;
    }
    ...
    public function getDetails($ids){
        ...
        if(count($ids) > 0){
            $result =JMutil::retryClient('JumeiProduct_Search_Read_Deals', 'getMallDetailForSearch', array($ids));
            return $result;
        }
        else return null;
    }
    ...
    public function detail2Product($mall_products_detail,$result,$ids){
        ...
        if(!empty($product)) {
            ...
            $product = JMutil::two_dimensional_array_merge(Promotion::getPromotion($ids, "mall"), $product);

            //旗舰店系列
            require_once(PROJECT_ROOT . '/util/Series.php');
            $params = JMutil::array_productId_storeId($product);
            //$product = JMutil::two_dimensional_array_merge(SeriesTransform::getSeries($ids), $product);
    ...
    public function  read_original_data(){
        ...
        $fp = fopen($tmp_file, "w");
        if (!$fp) {
            error_log("时间" . date("Y-m-d|H:i:s") . "Failed to open file {$tmp_file}", 3, "/home/logs/data_import/mall_product_import_error.log");
            return array();
        }
        fwrite($fp, "<add>");
        ...
        $file = $file_path . "/" . "mall_product_full_import_" . $time . ".xml";
        rename($tmp_file, $file);
        ImportWrite::update_timestamp('mall_product', $time);
        return true;
    }
    ...
    public function field_transform($data) {
        ...
        foreach ($data as $id => $product) {
            $data_transformed[$id]['doc_id'] = 'mall_product_' . $product['product_id'];
            $data_transformed[$id]['doc_type'] = 'mall_product';
            ...
            $data_transformed[$id]['jumei_price'] = ProductPropertyTransform::jumei_price_transform($product,"mall_product");
...
//        //写文件的时候，文件名是不一样的；
//        global $CONFIG;
//        $file_path = $CONFIG['file']['path']['mall_product'];
//        if (!is_dir($file_path)) {
...
//            error_log("时间" . $time . "商城全量索引失败！！！", 3, "/home/logs/data_import/mall_product_import_error.log");
//        }else{
//            //将本次索引时间记录下来
...
//            }
//
//            if(!isset($last_time['mall_product'])){
//                $last_time['mall_product'] = "mall_product=".$time;
//            }
