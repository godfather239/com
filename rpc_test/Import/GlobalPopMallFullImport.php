...
class GlobalPopMallFullImport implements LoadSourceData
{
    public function getIds($flag,$num=50){
        $hash_ids =JMutil::retryClient('JumeiProduct_Search_Read_Deals', 'getGlobalPOPMallDataWithMallIdAndLimit', array($flag,$num));
        ImportWrite::log_info("type:global_pop_mall,flag:{$flag},num:{$num},ids:{$hash_ids}");
        return $hash_ids;
    }
    ...
    public function getDetails($ids){
        ...
        if(count($ids)>0){
            $result =JMutil::retryClient('JumeiProduct_Search_Read_Deals', 'getGlobalPOPMallDetailForSearch', array($ids));
            return $result;
        }
        else return null;
    }

    public function detail2Product($global_pop_mall_detail,$result,$ids){
        $result=html_entity_decode($result);
        $product = json_decode($result,true);
        if(!empty($product)) {
            // 促销规则
            require_once(PROJECT_ROOT . '/util/Promotion.php');
            $product = JMutil::array_merge_for_globalmall(Promotion::getPromotion($ids, "globalmallmedia"), $product);
            // 专场信息
            ...
            foreach($mall_id_array as $mids){
                ...
                $global_pop_mall_detail += $this->field_transform($product);
            }
        }
        return $global_pop_mall_detail;
    }
    ...
    public function read_original_data() {
        ...
        $fp = fopen($tmp_file, "w");
        if (!$fp) {
            error_log("时间" . date("Y-m-d|H:i:s") . "Failed to open file {$tmp_file}", 3, "/home/logs/data_import/global_pop_mall_import_error.log");
            return array();
        }
        fwrite($fp, "<add>");
        ...
        while($flag!=-1){
            ...
            if($hash_ids=="[{\"minId\":".$flag."},[]]"){
                ...
                if(!empty($hash_ids_arrays)) {
                    ...
                    $global_pop_mall_detail=$this->detail2Product($global_pop_mall_detail,$result,$ids);
                    $xml = ImportWrite::get_xml_body($global_pop_mall_detail);
                    if (!empty($xml)) {
                        fwrite($fp, $xml);
                    }
                }
        ...
        $file = $file_path . "/" . "global_pop_mall_full_import_" . $time . ".xml";
        rename($tmp_file, $file);
        ImportWrite::update_timestamp('global_pop_mall', $time);
        return true;
    }
    ...
    public function field_transform($data) {
        ...
        foreach ($data as $id => $product) {
            $data_transformed[$id]['doc_id'] = 'global_pop_mall_' . $product['mall_id'];
            $data_transformed[$id]['doc_type'] = 'global_pop_mall';
            ...
            $data_transformed[$id]['jumei_price'] = ProductPropertyTransform::jumei_price_transform($product,"global_pop_mall");
...
//        //写文件的时候，文件名是不一样的；
//        global $CONFIG;
//        $file_path = $CONFIG['file']['path']['global_pop_mall'];
//        if (!is_dir($file_path)) {
...
//            error_log("时间" . $time . "global_pop_mall全量索引失败！！！", 3, "/home/logs/data_import/global_pop_mall_import_error.log");
//        }else{
//            //将本次索引时间记录下来
...
//            }
//
//            if(!isset($last_time['global_pop_mall'])){
//                $last_time['global_pop_mall'] = "global_pop_mall=".$time;
//            }
